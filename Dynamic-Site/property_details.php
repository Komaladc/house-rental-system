	...
	if(!isset($_GET['adid']) || $_GET['adid'] == NULL){
		...
		if($getAd){
			...
			if($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['sendmsg'])){
				...
				error_log("Message form submitted - adId: $adId, ownerId: $ownerId, renterId: $renterId");
				error_log("POST data: " . print_r($_POST, true));
				error_log("User login status: " . (Session::get("userlogin") ? "logged in" : "not logged in"));
				
				$sendNotif = $ntf->notificationInsert($adId, $ownerId, $renterId, $_POST);
				
				// Debug: Log result
				error_log("Notification result: " . $sendNotif);
			}
		}
	}
?>
...
<div class="container">
	...
	<!--Property Image Section------------->
		...
		<div class="property_small_details">
			...
			<h4><i class="fa-solid fa-rupee-sign"></i>NPR <?php echo $ad['adRent'];?>/<?php echo $ad['rentType'];?> <?php if(!empty($ad['adNegotiable'])){ ?><span>(negotiable)</span><?php } ?></h4>
		</div>
