...
/*Send client message process*/
	...
	if(empty($adId) || empty($ownerId) || empty($renterId)){
			...
			error_log("Notification Insert - adId: $adId, ownerId: $ownerId, renterId: $renterId");
			error_log("Notification Insert - name: $notfName, email: $notfEmail, phone: $notfPhone");
			error_log("Notification Insert - address: $notfAddress, message: $notfMsg");
			...
			// Debug: Log the query
			error_log("Notification Insert Query: " . $query);
			
			$notifinsert = $this->db->insert($query);
			if($notifinsert){
				error_log("Notification insert successful - ID: " . mysqli_insert_id($this->db->link));
				$msg = "<div class='alert alert_success'>Message Sent</div>";
				return $msg;
			} else{
				error_log("Notification insert failed - Error: " . mysqli_error($this->db->link));
